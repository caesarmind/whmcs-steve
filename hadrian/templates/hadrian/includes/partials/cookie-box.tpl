 ...
 * theme-wide: client area, the hadrian_cart order form (which uses this same
 * footer shell), and full-bleed pages (login). Gated on the admin "Cookie Box"
 * toggle ($hadrian.addonSettings.cookie_box) — emits nothing when off, so an
 ...
 *}
{if $hadrian.addonSettings.cookie_box}
    {assign var=mtCookiePos value=$hadrian.addonSettings.cookie_box_position|default:'bottom-left'}
    {assign var=mtCookieMsg value=$hadrian.addonSettings.cookie_box_message|default:''}
    {assign var=mtCookieBtn value=$hadrian.addonSettings.cookie_box_button|default:$LANG.continue}
