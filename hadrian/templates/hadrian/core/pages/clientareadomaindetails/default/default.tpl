...
<link rel="stylesheet" href="{$WEB_ROOT}/templates/{$template}/assets/css/pages/clientareadomaindetails.css?v={$hadrian.version|default:'1.0'}">
...
<div class="dd-alert {if $unpaidInvoiceOverdue|default:false}dd-alert-error{else}dd-alert-warn{/if}">
    ...
    <div>{$unpaidInvoiceMessage|default:$hadrianLang.domains.unpaidInvoice}</div>
    <a href="{$WEB_ROOT}/viewinvoice.php?id={$unpaidInvoice|escape}" class="btn-primary dd-alert-cta">{$LANG.payInvoice}</a>
</div>
{/if}
...
{if !empty($registrarcustombuttonresult)}
    {if $registrarcustombuttonresult == 'success'}
        <div class="dd-alert dd-alert-success">
            ...
            <div>{$LANG.moduleactionsuccess}</div>
        </div>
    {else}
        <div class="dd-alert dd-alert-error">
            ...
            <div>{$LANG.moduleactionfailed}</div>
        </div>
    {/if}
{/if}
...
<div class="dd-alert dd-alert-warn">
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/></svg>
    <div>{$LANG.domainCannotBeManagedUnlessActive}</div>
</div>
{/if}
    ...
    <div class="dd-main">
        ...
        <div class="card" style="padding:0;">
            <div class="dd-tabs" role="tablist">
                <button type="button" class="dd-tab active" data-dd-tab="overview">{$LANG.overview}</button>
                <button type="button" class="dd-tab" data-dd-tab="autorenew">{$LANG.domainsautorenew}</button>
                {if isset($managementoptions.nameservers) && $managementoptions.nameservers}<button type="button" class="dd-tab" data-dd-tab="nameservers">{$LANG.domainnameservers}</button>{/if}
                {if isset($managementoptions.locking) && $managementoptions.locking}<button type="button" class="dd-tab" data-dd-tab="reglock">{$LANG.domainregistrarlock}</button>{/if}
                {if !empty($addons.idprotection) || !empty($addons.dnsmanagement) || !empty($addons.emailforwarding)}<button type="button" class="dd-tab" data-dd-tab="addons">{$LANG.domainaddons}</button>{/if}
            </div>
            ...
            <div class="dd-tab-panel active" data-dd-panel="overview">
                <div class="dd-info-grid">
                    ...
                    <div class="dd-info-row">
                        <span class="dd-info-label">{$LANG.clientareahostingdomain}</span>
                    ...
                    <div class="dd-info-row">
                        <span class="dd-info-label">{$LANG.firstpaymentamount}</span>
                        <span class="dd-info-value">{$firstpaymentamount|escape}</span>
                    </div>
                    {/if}
                    {if !empty($registrationdate)}
                    <div class="dd-info-row">
                        <span class="dd-info-label">{$LANG.clientareahostingregdate}</span>
                        <span class="dd-info-value">{$registrationdate|escape}</span>
                    </div>
                    {/if}
                    {if !empty($recurringamount)}
                    <div class="dd-info-row">
                        <span class="dd-info-label">{$LANG.recurringamount}</span>
                        <span class="dd-info-value">{$recurringamount|escape}{if !empty($registrationperiod)} / {$registrationperiod|escape} {$LANG.orderyears}{/if}</span>
                    </div>
                    {/if}
                    {if !empty($nextduedate)}
                    <div class="dd-info-row">
                        <span class="dd-info-label">{$LANG.clientareahostingnextduedate}</span>
                        <span class="dd-info-value">{$nextduedate|escape}</span>
                    </div>
                    {/if}
                    {if !empty($paymentmethod)}
                    <div class="dd-info-row">
                        <span class="dd-info-label">{$LANG.orderpaymentmethod}</span>
                ...
                <div class="dd-ssl">
                    <div class="dd-ssl-row">
                        <span class="dd-info-label">{$LANG.sslState.sslStatus}</span>
                ...
                <div class="dd-quickactions">
                    <h3 class="dd-section-title">{$LANG.doToday}</h3>
            ...
            <div class="dd-tab-panel" data-dd-panel="autorenew">
                ...
                {/if}
                <p class="dd-section-text">{$LANG.domainrenewexp}</p>

                <div class="dd-status-row">
                    <span class="dd-info-label">{$LANG.domainautorenewstatus}</span>
                    <span class="dd-status-tag {if $autorenew}dd-tag-on{else}dd-tag-off{/if}">
                        {if $autorenew}{$LANG.domainsautorenewenabled}{else}{$LANG.domainsautorenewdisabled}{/if}
                    </span>
                </div>
                ...
                <form method="post" action="{$smarty.server.PHP_SELF}?action=domaindetails#tabAutorenew" class="dd-toggle-form">
                    ...
                    {if $autorenew}
                        ...
                        <button type="submit" class="btn-secondary dd-danger-btn">{$LANG.domainsautorenewdisable}</button>
                    {else}
                        <input type="hidden" name="autorenew" value="enable">
                        <button type="submit" class="btn-primary">{$LANG.domainsautorenewenable}</button>
                    {/if}
                </form>
            </div>
            ...
            <div class="dd-tab-panel" data-dd-panel="nameservers">
                ...
                <p class="dd-section-text">{$LANG.domainnsexp}</p>
                ...
                <form method="post" action="{$smarty.server.PHP_SELF}?action=domaindetails#tabNameservers" class="dd-form">
                    ...
                    <div class="dd-ns-grid">
                        {for $num=1 to 5}
                            <div class="dd-ns-row">
                                <label for="inputNs{$num}" class="dd-ns-label">{$LANG.clientareanameserver} {$num}</label>
            ...
            <div class="dd-tab-panel" data-dd-panel="reglock">
                ...
                <p class="dd-section-text">{$LANG.domainlockingexp}</p>
                ...
                <form method="post" action="{$smarty.server.PHP_SELF}?action=domaindetails#tabReglock" class="dd-toggle-form">
                    ...
                    {if $lockstatus == 'locked'}
                        <button type="submit" class="btn-secondary dd-danger-btn">{$LANG.domainreglockdisable}</button>
                    {else}
                        <button type="submit" name="reglock" value="1" class="btn-primary">{$LANG.domainreglockenable}</button>
                    {/if}
                </form>
            </div>
            {/if}
            ...
            <div class="dd-tab-panel" data-dd-panel="addons">
                <p class="dd-section-text">{$LANG.domainaddonsinfo}</p>
                ...
                <div class="dd-addon-list">
                    ...
                    <div class="dd-addon">
                        ...
                        <div class="dd-addon-body">
                            <h4 class="dd-addon-name">{$LANG.domainidprotection}</h4>
                            <p class="dd-addon-desc">{$LANG.domainaddonsidprotectioninfo}</p>
                        </div>
                        <div class="dd-addon-action">
                            <form action="{$WEB_ROOT}/clientarea.php?action=domainaddons" method="post">
                                ...
                                {if !empty($addonstatus.idprotection)}
                                    ...
                                    <span class="dd-addon-tag dd-tag-on">{$LANG.enabled}</span>
                                    <button type="submit" class="btn-secondary dd-addon-btn">{$LANG.disable}</button>
                                {else}
                                    <input type="hidden" name="buy" value="idprotect">
                                    <button type="submit" class="btn-primary dd-addon-btn">{$LANG.domainaddonsbuynow} {$addonspricing.idprotection|default:''|escape}</button>
                                {/if}
                            </form>
                        </div>
                    </div>
                    ...
                    <div class="dd-addon">
                        ...
                        <div class="dd-addon-body">
                            <h4 class="dd-addon-name">{$LANG.domainaddonsdnsmanagement}</h4>
                            <p class="dd-addon-desc">{$LANG.domainaddonsdnsmanagementinfo}</p>
                        </div>
                        <div class="dd-addon-action">
                            <form action="{$WEB_ROOT}/clientarea.php?action=domainaddons" method="post">
                                <input type="hidden" name="token" value="{$token|default:''|escape}">
                                ...
                                {if !empty($addonstatus.dnsmanagement)}
                                    ...
                                    <a class="btn-primary dd-addon-btn" href="{$WEB_ROOT}/clientarea.php?action=domaindns&domainid={$domainid|escape}">{$LANG.manage}</a>
                                    <button type="submit" class="btn-secondary dd-addon-btn">{$LANG.disable}</button>
                                {else}
                                    <input type="hidden" name="buy" value="dnsmanagement">
                                    <button type="submit" class="btn-primary dd-addon-btn">{$LANG.domainaddonsbuynow} {$addonspricing.dnsmanagement|default:''|escape}</button>
                                {/if}
                            </form>
                        </div>
                    </div>
                    ...
                    <div class="dd-addon">
                        ...
                        <div class="dd-addon-body">
                            <h4 class="dd-addon-name">{$LANG.domainemailforwarding}</h4>
                            <p class="dd-addon-desc">{$LANG.domainaddonsemailforwardinginfo}</p>
                        </div>
                        <div class="dd-addon-action">
                            <form action="{$WEB_ROOT}/clientarea.php?action=domainaddons" method="post">
                                <input type="hidden" name="token" value="{$token|default:''|escape}">
                                ...
                                {if !empty($addonstatus.emailforwarding)}
                                    ...
                                    <a class="btn-primary dd-addon-btn" href="{$WEB_ROOT}/clientarea.php?action=domainemailforwarding&domainid={$domainid|escape}">{$LANG.manage}</a>
                                    <button type="submit" class="btn-secondary dd-addon-btn">{$LANG.disable}</button>
                                {else}
                                    <input type="hidden" name="buy" value="emailfwd">
                                    <button type="submit" class="btn-primary dd-addon-btn">{$LANG.domainaddonsbuynow} {$addonspricing.emailforwarding|default:''|escape}</button>
                                {/if}
                            </form>
                        </div>
                    </div>
