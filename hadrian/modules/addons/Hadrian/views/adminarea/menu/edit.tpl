...
<form id="menuForm" method="post" action="?module=Hadrian&action=menu&sub=save">
    ...
    <div class="mt-toolbar">
        <a class="mt-back" href="?module=Hadrian&action=menu&tab={$menu->location|escape}">
        ...
        {if $flash == 'empty-payload-rejected'}
            <span class="mt-flash mt-flash-warn">
                ...
                <a href="?module=Hadrian&action=menu&sub=seed" style="text-decoration:underline">Re-seed presets</a>
...
(function(){
    ...
    function on(id, evt, fn) {
        ...
        else console.warn('[Hadrian menu] #' + id + ' not found — skipping ' + evt);
    }
    ...
    on('menuForm', 'submit', function(e){
        ...
        if (state.pristine) {
            console.log('[Hadrian menu] submit (pristine) — skipping JS regen, server inputs submit as-is');
            return;
        }

        try { syncStateFromDom(); } catch (err) { console.error('[Hadrian menu] syncStateFromDom failed', err); }
        ...
        try {
            ...
            console.error('[Hadrian menu] payload build failed — submit blocked', err);
            e.preventDefault();
        ...
        if (payload.length === 0) {
            ...
            if (existing > 0) {
                ...
                console.error('[Hadrian menu] empty payload while DOM still has', existing, 'items — submit blocked');
                return;
            }
        }
        ...
        console.group('[Hadrian menu] submit (dirty)');
