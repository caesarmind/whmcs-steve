...
final class Menus
{
    public function up(): void
    {
        ...
        if (!$schema->hasTable('hadrian_menus')) {
            $schema->create('hadrian_menus', function ($t) {
                $t->increments('id');
                $t->string('name', 191);
        ...
        if (!$schema->hasTable('hadrian_menu_items')) {
            $schema->create('hadrian_menu_items', function ($t) {
                $t->increments('id');
                ...
                $t->foreign('menu_id')
                    ->references('id')->on('hadrian_menus')
                    ->onDelete('cascade');

                $t->index(['menu_id', 'parent_id', 'position'], 'hadrian_menu_items_tree_idx');
            });
        }
    }
    ...
    public function down(): void
    {
        $schema = \WHMCS\Database\Capsule::schema();
        if ($schema->hasTable('hadrian_menu_items')) {
            $schema->drop('hadrian_menu_items');
        }
        if ($schema->hasTable('hadrian_menus')) {
            $schema->drop('hadrian_menus');
        }
    }
}
