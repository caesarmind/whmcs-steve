...
final class Uploader
{
     ...
     * @param string $webPath Web-relative path to the same dir (e.g. /templates/hadrian/assets/img/branding).
     */
    public function __construct(?string $diskDir = null, ?string $webPath = null)
    {
        $this->diskDir = $diskDir ?? self::defaultDiskDir();
        $this->webPath = '/' . trim($webPath ?? '/templates/hadrian/assets/img/branding', '/');
    }
     ...
     * "/templates/hadrian/assets/img/branding/foo.png"; new installs store
     ...
     *
     * Called from Hadrian_activate so the security posture is in place
    ...
    private function ensureHtaccess(): void
    {
        ...
        $rules = <<<HTACCESS
# Auto-generated by Hadrian — disables script execution in the branding dir.
