...
final class MenuController extends AbstractController
{
    ...
    {
        $key     = 'hadrian_menu_migration_version';
        ...
        try {
            ...
            if ($count > 0) {
                error_log("Hadrian menu: auto-migrated {$count} custom_link items to whmcs_page");
            }
        } catch (\Throwable $e) {
            error_log('Hadrian menu page migration failed: ' . $e->getMessage());
        }
    }
     ...
     *   - Migrator tracks executed migrations in `hadrian_migrations` and
    ...
    private function ensureMenuTables(): void
    {
        try {
            ...
            error_log('Hadrian menu self-heal failed: ' . $e->getMessage());
        }
    }
    ...
    public function saveAction(): string
    {
        ...
        if ($maxInputVars - 100 < $terminalCount) {
            error_log(sprintf(
                '[Hadrian saveAction] REFUSED save — max_input_vars too low. '
                . 'limit=%d, terminal_count=%d, menu_id=%d',
                $maxInputVars, $terminalCount, $menu->id
            ));
            $this->redirect(sprintf(
                '?module=Hadrian&action=menu&sub=edit&id=%d&flash=max-input-vars-exceeded&limit=%d&count=%d',
        ...
        error_log(sprintf(
            '[Hadrian saveAction] menu_id=%d source=%s items_count=%d post_keys=%s items_json_len=%d max_input_vars=%s',
            $menu->id,
            $usedSource,
            count($payload),
        ...
        if (empty($payload) && $existingCount > 0) {
            error_log('Hadrian menu save: refused to wipe menu items because '
                . 'payload was empty. menu_id=' . $menu->id
                . ' raw=' . substr($raw, 0, 200));
            $this->redirect('?module=Hadrian&action=menu&sub=edit&id='
                . $menu->id . '&flash=empty-payload-rejected');
        }

        $this->persistItems($menu->id, $payload);

        $this->redirect('?module=Hadrian&action=menu&sub=edit&id=' . $menu->id . '&flash=saved');
    }
    ...
    public function deleteAction(): string
    {
        ...
        $this->redirect('?module=Hadrian&action=menu&tab=' . urlencode($location));
    }
    ...
    public function seedAction(): string
    {
        ...
        $this->redirect('?module=Hadrian&action=menu&flash=seeded-' . $seeded);
    }
    ...
    public function createAction(): string
    {
        ...
        $this->redirect('?module=Hadrian&action=menu&sub=edit&id=' . $menu->id . '&flash=created');
    }

    public function resetDefaultsAction(): string
    {
        $reset = (new Seeder())->resetWhmcsDefaults();
        $this->redirect('?module=Hadrian&action=menu&flash=reset-defaults-' . $reset);
    }
    ...
    private function persistItems(int $menuId, array $payload): void
    {
        ...
        // separate table — they're not (single hadrian_menu_items table),
        ...
        error_log('[Hadrian persistItems] ' . json_encode($log, JSON_UNESCAPED_SLASHES));
    }
