...
function toggleProfileDropdown(e) {
    ...
    const sideDd = document.getElementById('sidebarProfileDropdown');
    if (sideDd) sideDd.classList.remove('open');
    if (dropdown) dropdown.classList.toggle('open');
}

// Profile dropdown toggle (sidebar user card)
function toggleSidebarProfile(e) {
    e.stopPropagation();
    const dropdown = document.getElementById('sidebarProfileDropdown');
    const topDd = document.getElementById('profileDropdown');
    if (topDd) topDd.classList.remove('open');
    if (dropdown) dropdown.classList.toggle('open');
}

// Close dropdowns on outside click
document.addEventListener('click', function(e) {
    ['profileDropdown', 'sidebarProfileDropdown'].forEach(function(id) {
...
(function() {
    ...
    // Remove all active states (flat items + grouped sub-items)
    document.querySelectorAll('.sidebar-item.active, .sidebar-group-items a.active').forEach(el => el.classList.remove('active'));
    ...
    if (navId) {
        ...
        if (el) {
            ...
            // If the active item lives inside a collapsible group, open it
...
// Sidebar group toggle — expand/collapse on click, persist state in localStorage
(function() {
    var KEY = 'apple-sidebar-groups';
    var saved;
    try { saved = JSON.parse(localStorage.getItem(KEY) || '[]'); } catch (e) { saved = []; }
    document.querySelectorAll('.sidebar-group').forEach(function(group) {
        var id = group.dataset.group;
        if (id && saved.indexOf(id) !== -1) group.classList.add('open');
        var toggle = group.querySelector('.sidebar-group-toggle');
        if (!toggle) return;
        toggle.addEventListener('click', function() {
            group.classList.toggle('open');
            var openIds = Array.prototype.slice.call(document.querySelectorAll('.sidebar-group.open'))
...
function toggleNotifications(e) {
    ...
    var sideProfile = document.getElementById('sidebarProfileDropdown');
    if (sideProfile) sideProfile.classList.remove('open');
}
