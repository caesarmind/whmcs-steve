...
The 137-line `mytheme_cart/viewcart.tpl` that mounted the SPA. Drops
into `mytheme_cart/viewcart.tpl` to switch back to the SPA path.
...
`mytheme/templates/mytheme/assets/css/apple-theme.css`. Harmless
when no `.vc-*` markup exists -- left in place so this archive is
self-contained.

## To re-enable the SPA path

1. Copy `viewcart.spa-shell.tpl` over `mytheme_cart/viewcart.tpl`.
2. Append the contents of `apple-theme-spa-bridge.js` to the bottom
   of `mytheme/templates/mytheme/assets/js/apple-theme.js`.
