{*
 * mytheme_cart/configureproduct.tpl
 *
 * Apple visual shell over standard_cart's exact configureproduct contract.
 *
 * What "exact contract" means here:
 *   - Form id `#frmConfigureProduct` with the two hidden inputs
 *     (configure=true, i={$i}) that the cart-controller PHP requires
 *     to fold this submission into the right cart slot.
 *   - Every element id scripts.min.js / inline JS binds to is
 *     preserved verbatim:
 *       #inputBillingcycle  — drives updateConfigurableOptions()
 *       #containerProductValidationErrors
 *       #containerProductValidationErrorsList
 *       #inputHostname  #inputRootpw  #inputNs1prefix  #inputNs2prefix
 *       #inputConfigOption{id}  (configoption[ID] input names)
 *       #productConfigurableOptions  #productAddonsContainer
 *       #orderSummary  #orderSummaryLoader  #producttotal
 *       #btnCompleteProductConfig
 *       #modalMetricPricing-{$systemName}
 *   - The .panel-addon / .panel-addon-selected classes that the JS
 *     toggles when an addon checkbox flips state are kept.
 *   - ionRangeSlider hookup for $configoption.optiontype eq 4 with
 *     qtymaximum is kept (matches scripts.min.js's expectation that
 *     #inputConfigOption{id} is the slider input).
 *   - recalctotals() and updateConfigurableOptions() inline calls
 *     preserved — they live in scripts.min.js.
 *
        ...
        <div class="cart-body">
            ...
            <form id="frmConfigureProduct">
                ...
                <div class="row">
                    ...
                    {* ═══════════════════════════════════════════════
                       LEFT — product info + config form
                       ═══════════════════════════════════════════════ *}
                    <div class="secondary-cart-body">

                        {* ─── Apple product header card ─── *}
                        ...
                        {* ─── Server fields (type=server only) ─── *}
                        ...
                        <div class="alert alert-info info-text-sm">
                            ...
                            <a href="{$WEB_ROOT}/contact.php" target="_blank" class="alert-link">
