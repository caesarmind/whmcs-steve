{* =========================================================================
   Password strength meter. Attaches to #inputNewPassword1 / #inputNewPassword2
   and renders a progress bar in #passwordStrengthBar.

   Apple CSS styles .progress and .progress-bar to match the theme.
   ========================================================================= *}
<div class="progress pw-strength-bar" id="passwordStrengthBar">
...
<script>
    ...
    function validatePassword2() {
        ...
        if (password2 && password1 !== password2) {
            password2Input.removeClass('is-valid').addClass('is-invalid');
            jQuery("#inputNewPassword2Msg").html('<p class="form-hint" id="nonMatchingPasswordResult">{"{lang key='pwdoesnotmatch'}"|escape}</p>');
            {if !isset($noDisable)}jQuery('input[type="submit"]').attr('disabled', 'disabled');{/if}
        } else {
            if (password2 && password1.length <= maximumPasswordLength) {
                password2Input.removeClass('is-invalid').addClass('is-valid');
    ...
    jQuery(document).ready(function() {
        ...
        jQuery("#inputNewPassword2").keyup(function() { validatePassword2(); });
    });
</script>
