...
<div class="page-header">
    <h1 class="page-title">{lang key='clientareanavinvoices'}</h1>
</div>
...
<script>
    jQuery(document).ready(function() {
        ...
        {if $orderby == 'default'}table.order([4, 'desc'], [2, 'asc']);
        {elseif $orderby == 'invoicenum'}table.order(0, '{$sort}');
        {elseif $orderby == 'date'}table.order(1, '{$sort}');
        {elseif $orderby == 'duedate'}table.order(2, '{$sort}');
        {elseif $orderby == 'total'}table.order(3, '{$sort}');
        {elseif $orderby == 'status'}table.order(4, '{$sort}');{/if}
        table.draw();
...
<div class="card">
    <div class="card-body">
        <div class="table-container">
            <table id="tableInvoicesList" class="table table-hover w-hidden">
                <thead>
                    <tr>
                ...
                <tbody>
                    {foreach $invoices as $invoice}
                        <tr onclick="clickableSafeRedirect(event, 'viewinvoice.php?id={$invoice.id}', false)">
                            ...
                            <td><span class="status-pill {$invoice.statusClass}">{$invoice.status}</span></td>
                        </tr>
                    {/foreach}
                </tbody>
            </table>
