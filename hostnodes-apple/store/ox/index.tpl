{* store/ox/index.tpl — Open-Xchange email landing. *}
<div class="page-header">
    <h1 class="page-title">{lang key='store.ox.title'}</h1>
    <p class="page-subtitle">{lang key='store.ox.subtitle'}</p>
</div>
    ...
    {foreach $packages as $package}
        <div class="store-product-card card">
            ...
            <div class="card-body">
                ...
                <a href="{routePath('cart-add', 'ox', $package.id)}" class="btn btn-primary btn-full">{lang key='orderNow'}</a>
            </div>
        </div>
    {/foreach}
</div>
