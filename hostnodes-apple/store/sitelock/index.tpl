{* store/sitelock/index.tpl — SiteLock landing. *}
<div class="page-header">
    <h1 class="page-title">{lang key='store.sitelock.title'}</h1>
    <p class="page-subtitle">{lang key='store.sitelock.subtitle'}</p>
</div>
    ...
    {foreach $packages as $package}
        <div class="store-product-card card">
            ...
            <div class="card-body">
                ...
                <a href="{routePath('cart-add', 'sitelock', $package.id)}" class="btn btn-primary btn-full">{lang key='orderNow'}</a>
            </div>
        </div>
    {/foreach}
</div>
