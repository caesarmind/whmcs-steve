{* store/sitelockvpn/index.tpl — SiteLock VPN landing. *}
<div class="page-header">
    <h1 class="page-title">{lang key='store.sitelockvpn.title'}</h1>
    <p class="page-subtitle">{lang key='store.sitelockvpn.subtitle'}</p>
</div>
    ...
    {foreach $packages as $package}
        <div class="store-product-card card">
            ...
            <div class="card-body">
                ...
                <a href="{routePath('cart-add', 'sitelockvpn', $package.id)}" class="btn btn-primary btn-full">{lang key='orderNow'}</a>
            </div>
        </div>
    {/foreach}
</div>
