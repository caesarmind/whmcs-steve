{* store/weebly/upgrade.tpl — Weebly plan upgrade. *}
<div class="page-header">
    <h1 class="page-title">{lang key='store.weebly.upgrade.title'}</h1>
    <p class="page-subtitle">{lang key='store.weebly.upgrade.subtitle'}</p>
</div>
    ...
    {foreach $upgradePackages as $package}
        <div class="store-product-card card">
            ...
            <div class="card-body">
                ...
                <a href="{routePath('cart-upgrade', 'weebly', $package.id)}" class="btn btn-primary btn-full">{lang key='upgrade'}</a>
            </div>
        </div>
    {/foreach}
</div>
