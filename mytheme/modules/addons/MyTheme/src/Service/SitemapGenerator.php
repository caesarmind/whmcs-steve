 ...
 * the mytheme_pages registry (Models\Page) for static pages, and the same
...
{
    private const MARK_START = 'mytheme sitemap generator start';
    private const MARK_END   = 'mytheme sitemap generator end';
     ...
     * existing sitemap.xml is reused with the previous mytheme block stripped,
    ...
    public function buildSitemap(string $template, ?array $cfg = null): string
    {
        ...
        if (!empty($cfg['productGroups']) && class_exists('\\WHMCS\\Product\\Group')) {
            try {
                ...
                error_log('MyTheme sitemap: product groups skipped — ' . $e->getMessage());
            }
        }
        ...
        if (!empty($cfg['announcements']) && class_exists('\\WHMCS\\Announcement\\Announcement') && function_exists('routePath')) {
            try {
                ...
                error_log('MyTheme sitemap: announcements skipped — ' . $e->getMessage());
            }
        }
        ...
        if (!empty($cfg['knowledgebase']) && function_exists('routePath')) {
            try {
                ...
                error_log('MyTheme sitemap: knowledge base skipped — ' . $e->getMessage());
            }
        }
        ...
        if (!empty($cfg['downloads']) && function_exists('routePath')) {
            try {
                ...
                error_log('MyTheme sitemap: downloads skipped — ' . $e->getMessage());
            }
        }
    ...
    /** Remove the previous mytheme block (markers + the <url> nodes between). */
