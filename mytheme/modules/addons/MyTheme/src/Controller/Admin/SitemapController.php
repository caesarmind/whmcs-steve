...
use MyTheme\Service\SitemapGenerator;
...
final class SitemapController extends AbstractController
{
    ...
    public function generateAction(): string
    {
        ...
        if ($template === null) {
            $this->redirect('?module=MyTheme&action=sitemap&err=' . rawurlencode('No active template.'));
        }
        try {
            $res = (new SitemapGenerator())->write($template->getName());
        } catch (\Throwable $e) {
            $this->redirect('?module=MyTheme&action=sitemap&err=' . rawurlencode('Generation failed: ' . $e->getMessage()));
        }
        $key = $res['ok'] ? 'flash' : 'err';
        $this->redirect('?module=MyTheme&action=sitemap&' . $key . '=' . rawurlencode($res['message']));
    }
