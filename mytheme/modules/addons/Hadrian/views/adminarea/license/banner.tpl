...
<div class="alert alert--license alert--{if $state == 'DEV_MODE'}warning{elseif $state == 'EXPIRED' || $state == 'INVALID' || $state == 'CANCELLED' || $state == 'BANNED'}danger{else}info{/if}">
    <div class="alert-body">
        {if $state == 'DEV_MODE'}
            <strong>Hadrian — Development mode.</strong> {$message|escape}
        {elseif $message}
            <strong>Hadrian:</strong> {$message|escape}
        {elseif $state == 'EXPIRED'}
            <strong>Hadrian license expired.</strong> Please renew to keep using your theme.
        {elseif $days !== null && $days <= 14}
            <strong>Hadrian license expires in {$days} day{if $days != 1}s{/if}.</strong>
        {else}
            <strong>Hadrian:</strong> license attention needed.
        {/if}
    </div>
    <div class="alert-actions">
        ...
        {/if}
        <a href="addonmodules.php?module=Hadrian&action=license" class="btn btn-default">Manage</a>
    </div>
</div>
<style>
