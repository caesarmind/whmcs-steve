# Hadrian — WHMCS 9 Client Theme Starter (PHP 8.1+)
...
├── modules/addons/Hadrian/                          ← the engine (encode this)
...
└── templates/hadrian/                               ← the visual layer (mostly plain)
    ├── theme.json                                   ← MANIFEST (not filesystem scan)
    ├── core/hadrian.php                             ← per-template encoded config (secret_key)
...
- Marketing pages (`templates/hadrian/homepage.tpl` and friends)
...
| Settings | 4 stores: `tbladdonmodules`, `tblconfiguration`, `hadrian_settings`, JSON-in-text columns | 2 stores: `tblconfiguration` for WHMCS interop, one typed `hadrian_settings` table for the rest |
| Per-vendor store partials | Inside the theme (17+ vendor dirs) | Out of scope for v1 — buyers add via separate addons |

## Quick start

```bash
cd Hadrian-WHMCS-Starter
npm install
npm run build              # compiles SCSS, bundles JS, computes integrity hashes
# Then copy modules/addons/Hadrian/ → <whmcs>/modules/addons/Hadrian/
# And copy templates/hadrian/ → <whmcs>/templates/hadrian/
# Activate "Hadrian" under WHMCS Admin → Setup → Addon Modules
# Then choose Active Template = "hadrian" under Setup → General Settings → Client Area Template
```

## Renaming for your own brand

Two slugs to change everywhere:
- **Addon name** (`Hadrian` PascalCase) — appears in addon class names, function prefixes, namespace
- **Theme slug** (`hadrian` lowercase) — appears in directory names, config keys, route paths

```bash
# Replace placeholders (example for renaming to "Aurora")
grep -rl 'Hadrian' . | xargs sed -i '' 's/Hadrian/Aurora/g'
grep -rl 'hadrian' . | xargs sed -i '' 's/hadrian/aurora/g'
mv modules/addons/Hadrian   modules/addons/Aurora
mv templates/hadrian        templates/aurora
mv templates/aurora/core/hadrian.php templates/aurora/core/aurora.php
```
...
5. Read [docs/TEMPLATE-STRUCTURE.md](docs/TEMPLATE-STRUCTURE.md) for the `templates/hadrian` commercial visual-layer structure
