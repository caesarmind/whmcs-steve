...
<link rel="stylesheet" href="{$WEB_ROOT}/templates/{$template}/assets/css/pages/knowledgebasecat.css?v={$hadrian.version|default:'1.0'}">
...
<header class="page-header">
    <h1>{if isset($category.name)}{$category.name|escape}{else}{$LANG.knowledgebasetitle}{/if}</h1>
    {if isset($category.description) && $category.description}
    <p class="page-subtitle">{$category.description|strip_tags|truncate:240:"…"}</p>
    {else}
    <p class="page-subtitle">{$hadrianLang.support.kbCatSubtitle}</p>
    {/if}
</header>
    ...
    <div class="kb-main">
        <div class="card when-full" style="padding:0;">
            ...
            <form class="kb-cat-search" method="get" action="{$WEB_ROOT}/knowledgebase.php">
                ...
                <input type="search" name="search" placeholder="{$hadrianLang.support.searchInCategory}" value="{$kbsearchterm|default:''|escape}" autocomplete="off">
            </form>
            ...
            <a href="{$WEB_ROOT}/knowledgebase.php?action=displaycat&catid={$cat.id}" class="kb-row">
                ...
                <div class="kb-row-info">
                    ...
                    {if isset($cat.numarticles)}<div class="kb-row-sub">{$cat.numarticles|escape} {$hadrianLang.support.articlesCount}</div>{/if}
                </div>
            ...
            <a href="{$WEB_ROOT}/knowledgebase.php?action=displayarticle&id={$art.id}" class="kb-row">
                ...
                <div class="kb-row-info">
                    ...
                    {if isset($art.views) && $art.views}<div class="kb-row-sub">{$art.views|escape} {$hadrianLang.support.views}</div>{/if}
                </div>
                <span class="kb-row-chev"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg></span>
            </a>
            {/foreach}
        ...
        <div class="card when-empty">
            <div class="kb-empty">
                ...
                <p class="kb-empty-title">{$hadrianLang.support.kbNoResults}</p>
                <p class="kb-empty-sub">{$hadrianLang.support.kbNoResultsSub}</p>
