...
<nav class="homepage-nav only-top">
    <div class="homepage-nav-inner">
        ...
        {else}
            ...
            <a href="{$WEB_ROOT}/" class="nav-item">{$LANG.home}</a>
            {if $loggedin}<a href="{$WEB_ROOT}/clientarea.php" class="nav-item">{$LANG.clientareanavhome}</a>{/if}
        {/if}
        ...
        <a href="{$WEB_ROOT}/cart.php?a=view" class="topbar-btn topbar-cart-btn" title="{$LANG.carttitle}">
            ...
            {* Cart-count badge.
               ...
               MyTheme's ClientAreaPage hook (see hooks.php). $cartitems is
        ...
        {if $loggedin}
            <div class="profile-dropdown-wrapper only-in" id="navUserWrap">
                <div class="topbar-avatar" onclick="togglePortalProfile && togglePortalProfile(event)" title="{$LANG.accounttab}">{$user_initials|default:'U'}</div>
