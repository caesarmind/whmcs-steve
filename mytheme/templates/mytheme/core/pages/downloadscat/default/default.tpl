...
{assign var=catName value=$pagetitle|default:$LANG.downloadstitle}
...
<header class="page-header">
    ...
    <p class="page-subtitle">{$hadrianLang.support.downloadsCatIntro}</p>
    {if $hasFiles}
    <div class="page-header-meta">
        <span>{$downloads|@count} {$LANG.downloadsfiles}</span>
    </div>
    {/if}
</header>
        ...
        <div class="card" style="padding: 0;">
            ...
            <div class="dl-cat-banner">
                ...
                <div class="dl-cat-banner-meta">
                    ...
                    <div class="dl-cat-banner-sub">{$hadrianLang.support.downloadsCatBanner}</div>
                </div>
            </div>
            ...
            {foreach $dlcats as $sub}
                <a href="{routePath('download-by-cat', $sub.id, $sub.urlfriendlyname)}" class="dl-file-row">
                    ...
                    <div class="dl-file-meta">
                        ...
                        <div class="dl-file-sub"><span>{$sub.numarticles|default:0} {$LANG.downloadsfiles}</span></div>
                    </div>
                    <div class="dl-file-actions">
                        <span class="dl-dl-btn"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg>{$hadrianLang.common.open}</span>
                    </div>
                </a>
            {/foreach}
            {/if}
            ...
            <div class="dl-toolbar">
                <div class="dl-search">
                    ...
                    <input type="search" id="dlCatSearch" placeholder="{$hadrianLang.support.downloadsSearchInCat}" autocomplete="off">
                </div>
                <div class="dl-filters" role="tablist">
                    <button type="button" class="active" data-type="all">{$LANG.all}</button>
                    <button type="button" data-type="pdf">PDF</button>
                    <button type="button" data-type="zip">ZIP</button>
                    <button type="button" data-type="other">{$hadrianLang.support.otherType}</button>
                </div>
                <div class="dl-toolbar-spacer"></div>
                <select class="dl-sort" id="dlSort" aria-label="{$hadrianLang.common.sortBy}">
                    <option value="recent">{$hadrianLang.common.sortRecent}</option>
                    <option value="name">{$hadrianLang.common.sortAlpha}</option>
                </select>
            </div>
                    ...
                    <div class="dl-file-row" data-type="{$ft|escape}" data-name="{$download.title|strip_tags|lower|escape}" data-idx="{$download@index}">
                        ...
                        <div class="dl-file-meta">
                            <div class="dl-file-name">
                                ...
                                {if $download.isnew}<span class="dl-file-new-pill">{$LANG.new}</span>{/if}
                        ...
                        <div class="dl-file-actions">
                            ...
                            <a href="{$download.link|default:'#'}" class="dl-dl-btn"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>{$LANG.login}</a>
                            {else}
                            <a href="{$download.link|default:'#'}" class="dl-dl-btn"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>{$LANG.downloadbutton}</a>
                            {/if}
                        </div>
                    </div>
                {/foreach}
            ...
            <div class="dl-footer">
                <span>{$hadrianLang.support.showingCount} {$downloads|@count}</span>
                <div class="spacer"></div>
                <a href="{$WEB_ROOT}/downloads.php">{$hadrianLang.support.allDownloads}</a>
            </div>
            {/if}
...
<div class="when-empty">
    <div class="card">
        <div class="dl-empty">
            ...
            <p class="dl-empty-title">{$hadrianLang.support.downloadsNoneInCat}</p>
            <p class="dl-empty-sub">{$hadrianLang.support.downloadsCatEmpty}</p>
