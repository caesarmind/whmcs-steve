...
<div class="tfac-wrap">
    <div class="card tfac-card">
        ...
        <h1 class="tfac-title">{$LANG.twofactorauth|default:"Two-factor authentication"}</h1>

        {if $newbackupcode}
            <div class="tfac-alert tfac-alert-success">{$LANG.twofabackupcodereset|default:"Backup code accepted. New codes have been emailed to you."}</div>
        {elseif $incorrect}
            <div class="tfac-alert tfac-alert-error">{$LANG.twofa2ndfactorincorrect|default:"The code you entered is incorrect. Try again."}</div>
        {elseif isset($error) && $error}
            <div class="tfac-alert tfac-alert-error">{$error|strip_tags|escape}</div>
        {else}
            <p class="tfac-sub">{$LANG.twofa2ndfactorreq|default:"Enter the verification code from your authenticator app to continue."}</p>
        {/if}
        ...
        <form method="post" id="frmTwoFactorBackup" class="tfac-form{if !$usingBackup} tfac-hidden{/if}">
            <label class="form-label" for="tfac-backup">{$LANG.twofabackupcodelogin|default:"Backup code"}</label>
            <input type="text" name="twofabackupcode" id="tfac-backup" class="form-input tfac-backup-input" placeholder="xxxx-xxxx-xxxx" autocomplete="one-time-code">
            <button type="submit" class="btn-primary tfac-submit">{$LANG.loginbutton|default:"Sign in"}</button>
        </form>

        <div class="tfac-foot">
            <button type="button" id="tfacToggle" class="tfac-link">
                {if $usingBackup}{$LANG.usetotpinstead|default:"Use authenticator code instead"}{else}{$LANG.usebackupcode|default:"Use a backup code"}{/if}
            </button>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    toggle.addEventListener('click', function () {
        ...
        toggle.textContent = usingBackup ? 'Use a backup code' : 'Use authenticator code instead';
    });
});
{/literal}
</script>
