{* Store landing — SiteLock VPN bundle.
   ...
   NOTE: this is currently mounted at /store/sitelock (the original
   scaffold's slug). Nexus treats SiteLock (plain) and SiteLock-VPN as
   two distinct routes (store/sitelock vs store/sitelockvpn). If you
   want Nexus parity later, rename this directory to store-sitelockvpn
   and restore store-sitelock as the plain SiteLock landing.
